...
static void nav_event_input(struct gf_dev *gf_dev, gf_nav_event_t nav_event)
{
	...
	case GF_NAV_FINGER_DOWN:
		...
		input_report_key(gf_dev->input, KEY_KPENTER, 1);
		input_sync(gf_dev->input);
		break;

	case GF_NAV_FINGER_UP:
		pr_debug("%s nav finger up\n", __func__);
		input_report_key(gf_dev->input, KEY_KPENTER, 0);
		input_sync(gf_dev->input);
		break;
...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	if (GF_KEY_HOME == gf_key->key) {
		...
		pr_info("%s: received power_key event[%d], key=%d, value=%d\n",
	...
	if ((GF_KEY_POWER == gf_key->key || GF_KEY_RIGHT == gf_key->key ||
		GF_KEY_CAMERA == gf_key->key)
			&& (gf_key->value == 1)) {
			pr_info("%s: received key value == 1, event[%d], key=%d, value=%d\n",
			__func__, key_input, gf_key->key, gf_key->value);
