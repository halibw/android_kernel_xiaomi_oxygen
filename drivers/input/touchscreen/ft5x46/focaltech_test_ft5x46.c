...
unsigned char FT5X46_TestItem_RawDataTest(struct i2c_client *client)
{
	...
	ReCode = WriteReg(REG_DATA_SELECT, 0);

	if (ReCode != ERROR_CODE_OK) {
		FOCAL_TEST_DBG("Failed to write REG_DATA_SELECT! ");
		goto TEST_ERR;
	}
	...
	if (pdata->has_key) {
		for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
			for (iCol = 0; iCol < (g_ScreenSetParam.iRxNum - 1); iCol++) {
				...
				if (iValue < RawDataMin) {
					btmpresult = false;
					FOCAL_TEST_PRINT("rawdata failure.Node=(%d,%d),value=%d",
		...
		if (iValue < RawDataMin) {
			btmpresult = false;
			FOCAL_TEST_PRINT("rawdata failure.Node=(%d,%d),Get_value=%d",
				       4, iCol + 1, iValue);
		}
		...
		if (iValue < RawDataMin) {
			btmpresult = false;
			FOCAL_TEST_PRINT("rawdata test failure.Node=(%d,%d),Get_value=%d",
				       12, iCol + 1, iValue);
		}
	} else {
		for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
		...
		if (ReCode == ERROR_CODE_OK) {
			while (times++ < MaxTimes) {
				...
				if (ReCode == ERROR_CODE_OK) {
					...
					FOCAL_TEST_DBG("StartScan read DEVIDE_MODE_ADDR error.");
					break;
				}
			}
			...
			if (times < MaxTimes)
				...
				FOCAL_TEST_DBG("times NOT < MaxTimes. error.");
			}
		} else
			FOCAL_TEST_DBG("StartScan write DEVIDE_MODE_ADDR error.");
	} else
		FOCAL_TEST_DBG("StartScan read DEVIDE_MODE_ADDR error.");
...
unsigned char ReadRawData(unsigned char Freq, unsigned char LineNum, int ByteNum, int *pRevBuffer)
{
	...
	if (ReCode != ERROR_CODE_OK) {
		FOCAL_TEST_DBG("Failed to write REG_LINE_NUM! ");
		goto READ_ERR;
	}
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		if (ReCode != ERROR_CODE_OK) {
			FOCAL_TEST_DBG("read rawdata Comm_Base_IIC_IO Failed!1 ");
			goto READ_ERR;
		}
	}
	...
	for (i = 1; i < iReadNum; i++) {
		...
		if (i == iReadNum - 1) {
			...
			if (ReCode != ERROR_CODE_OK) {
				FOCAL_TEST_DBG("read rawdata Comm_Base_IIC_IO Failed!2 ");
				goto READ_ERR;
			}
		} else {
			focal_msleep(10);
			...
			if (ReCode != ERROR_CODE_OK) {
				FOCAL_TEST_DBG("read rawdata Comm_Base_IIC_IO Failed!3 ");
				goto READ_ERR;
			}
		}
	}
	...
	if (ReCode == ERROR_CODE_OK) {
		for (i = 0; i < (ByteNum >> 1); i++) {
			pRevBuffer[i] = (short) ((m_ucTempData[i << 1] << 8) + (m_ucTempData[(i << 1) + 1]));
		}
	}
READ_ERR:
	return ReCode;
...
static unsigned char GetChannelNum(void)
{
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		if (g_ScreenSetParam.iTxNum > g_ScreenSetParam.iUsedMaxTxNum) {
			FOCAL_TEST_DBG("Failed to get Tx number, Get num = %d, UsedMaxNum = %d",
				     g_ScreenSetParam.iTxNum, g_ScreenSetParam.iUsedMaxTxNum);
			g_ScreenSetParam.iTxNum = 0;
			return ERROR_CODE_INVALID_PARAM;
		}
	} else
		FOCAL_TEST_DBG("Failed to get Tx number");
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		if (g_ScreenSetParam.iRxNum > g_ScreenSetParam.iUsedMaxRxNum) {
			FOCAL_TEST_DBG("Failed to get Rx number, Get num = %d, UsedMaxNum = %d",
				     g_ScreenSetParam.iRxNum, g_ScreenSetParam.iUsedMaxRxNum);
			g_ScreenSetParam.iRxNum = 0;
			return ERROR_CODE_INVALID_PARAM;
		}
	} else
		FOCAL_TEST_DBG("Failed to get Rx number");
...
static unsigned char GetRawData(void)
{
	...
	if (0 == (g_ScreenSetParam.iTxNum + g_ScreenSetParam.iRxNum)) {
		...
		if (ERROR_CODE_OK != ReCode) {
			FOCAL_TEST_ERR("Error Channel Num...");
			return ERROR_CODE_INVALID_PARAM;
		}
	}

	FOCAL_TEST_DBG("Start Scan ...");
	ReCode = StartScan();

	if (ERROR_CODE_OK != ReCode) {
		FOCAL_TEST_DBG("Failed to Scan ...");
		return ReCode;
	}
...
static void ShowRawData(void)
{
	...
	for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
		FOCAL_TEST_PRINT("Tx%2d:  ", iRow + 1);

		for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
			FOCAL_TEST_PRINT("%5d    ", m_RawData[iRow][iCol]);
...
int FT5X46_TestItem_GetRawData(u8 *buff, int type)
{
	unsigned char ReCode;
	int j = 0;
	int index = 0;
	...
	ReCode = WriteReg(REG_DATA_SELECT, type);
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		case 0:
			...
			break;
		case 1:
			snprintf(buff, 8, "differ\n");
			break;
		default:
			FOCAL_TEST_ERR("\n Get Data unknown type%d", type);
			goto ERROR;
		}

		for (j = 0; j < g_ScreenSetParam.iRxNum * g_ScreenSetParam.iTxNum; j++) {
			if ((j + 1) % g_ScreenSetParam.iRxNum)
				snprintf(temp_buf, 7, "%5d ", m_iTempRawData[j]);
			else
				snprintf(temp_buf, 7, "%5d\n", m_iTempRawData[j]);

			strlcat(buff, temp_buf, 7 * g_ScreenSetParam.iRxNum * g_ScreenSetParam.iTxNum);
		}
	}
	return strlen(buff);
ERROR:
	snprintf(buff, 7, "error\n");
	if (temp_buf != NULL) {
		kfree(temp_buf);
		temp_buf = NULL;
	}
...
unsigned char FT5X46_TestItem_WeakShortTest(struct i2c_client *client)
{
	...
	if (ReCode != ERROR_CODE_OK) {
		FOCAL_TEST_ERR(" EnterWork failed.. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	...
	if (ReCode != ERROR_CODE_OK) {
		...
		FOCAL_TEST_DBG(" IcValue:0x%02x\n", IcValue);

	ReCode = EnterFactory();
	SysDelay(100);

	if (ReCode != ERROR_CODE_OK) {
		FOCAL_TEST_ERR(" EnterFactory failed.. Error Code: %d", ReCode);
		goto TEST_ERR;
	}
	...
	if (m_bV3TP) {
		...
		FOCAL_TEST_DBG("Newly acquired TxNum:%d, RxNum:%d", iTxNum, iRxNum);
	}

	if (ReCode != ERROR_CODE_OK) {
		FOCAL_TEST_ERR("ReCode  error. Error Code: %d\n", ReCode);
		goto TEST_ERR;
	}
	...
	if (i >= 5) {
		FOCAL_TEST_ERR("StartScan Failed for several times.!\n");
		goto TEST_ERR;
	}

	iAdcData = focal_malloc(iAllAdcDataNum * sizeof(int));
	memset(iAdcData, 0, iAllAdcDataNum);

	for (i = 0; i < 10; i++) {
		memset(iAdcData, 0, iAllAdcDataNum);
		FOCAL_TEST_DBG("WeakShort_GetAdcData times: %d", i);
		for (index = 0; index < 3; ++index)
	...
	if (i >= 10)	{
		FOCAL_TEST_ERR("WeakShort_GetAdcData or ADC data error. tried times: %d", i);
		goto TEST_ERR;
	}
	...
	for (i = 0; i < iAllAdcDataNum; i++) {
		if (i <= (iChannelNum + 1)) {
			if (i == 0)
				...
				FOCAL_TEST_PRINT("Ground %02d: %4d,\n", i, iAdcData[i]);
			else if (i <= (iMaxTx + 1))
				FOCAL_TEST_PRINT("Tx%02d: %4d,      ", i - 1, iAdcData[i]);
			else if (i <= (iChannelNum + 1))
				FOCAL_TEST_PRINT("Rx%02d: %4d,      ", i - iMaxTx - 1, iAdcData[i]);
			...
			if (i == (iChannelNum + 2))
				...
				FOCAL_TEST_PRINT("Tx%02d: %4d,       ", i - (iChannelNum + 2), iAdcData[i]);
			else if (i < iAllAdcDataNum)
				FOCAL_TEST_PRINT("Rx%02d: %4d,       ", i - iMaxTx - (iChannelNum + 2), iAdcData[i]);
	...
	FOCAL_TEST_PRINT("\r\n");
	FOCAL_TEST_DBG("");
	fMShortResistance = focal_malloc(iChannelNum * sizeof(int));
	memset(fMShortResistance, 0, iChannelNum);
	fGShortResistance = focal_malloc(iChannelNum * sizeof(int));
	memset(fGShortResistance, 0, iChannelNum);
	iMin_CG = pdata->imin_cg;
	iDoffset = iOffset - 1024;
	iDrefn = iClbData_Ground;
	FOCAL_TEST_DBG("Drefp:%5d	\r\n", iDrefn);
	FOCAL_TEST_DBG("Doffset:%5d\r\n", iDoffset);
	FOCAL_TEST_DBG("Rshort(Ground):\n\n\n");
	fKcal = 1;
	FOCAL_TEST_DBG("Short Circuit (Channel and Ground):\r\n");

	for (i = 0; i < iChannelNum; i++) {
		iDsen = iAdcData[i + 2];
		FOCAL_TEST_PRINT("%5d	", iDsen);
		...
		if (IcValue <= 0x05 || IcValue == 0xff)
			...
			if (iDrefn - iDsen <= 0) {
				...
				FOCAL_TEST_PRINT("%02d  ", fGShortResistance[i]);
				continue;
			}
		...
		if ((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0)) {
			...
			if (i + 1 <= iMaxTx)
				FOCAL_TEST_PRINT("Tx%02d: %02d (k¦¸),	", i + 1, fGShortResistance[i]);
			else
				FOCAL_TEST_PRINT("Rx%02d: %02d (k¦¸),	", i + 1 - iMaxTx, fGShortResistance[i]);
	...
	for (i = 0; i < iChannelNum; i++) {
		...
		if (IcValue <= 0x05 || IcValue == 0xff) {
			...
			if (iDrefn - iDsen <= 0) {
				...
				FOCAL_TEST_PRINT("%02d  ", fMShortResistance[i]);
				continue;
			}
		...
		if (fMShortResistance[i] <= 0  || fMShortResistance[i] < iMin_CC) {
			...
			if (i + 1 <= iMaxTx)
				FOCAL_TEST_PRINT("Tx%02d: %02d(k¦¸),	", i + 1, fMShortResistance[i]);
			else
				FOCAL_TEST_PRINT("Rx%02d: %02d(k¦¸),	", i + 1 - iMaxTx, fMShortResistance[i]);
	...
	if (bCapShortTest && iCount)
		FOCAL_TEST_DBG(" bCapShortTest && iCount.  need to add ......");
	...
	if (m_bV3TP) {
		...
		if (ReCode != ERROR_CODE_OK) {
			FOCAL_TEST_ERR("Failed to restore mapping type!\r\n");
			goto TEST_ERR;
		}
	...
	if (NULL != fMShortResistance) {
		focal_free(fMShortResistance);
		fMShortResistance = NULL;
	}

	if (NULL != fGShortResistance) {
		focal_free(fGShortResistance);
...
static unsigned char WeakShort_GetAdcData(int AllAdcDataLen, int *pRevBuffer)
{
	...
	bool bAdcOK = false;
	FOCAL_TEST_DBG("\n");
	pDataSend = focal_malloc(iReadDataLen + 1);
	...
	if (!bAdcOK) {
		FOCAL_TEST_ERR("ADC data NOT ready.  error.\n");
		ReCode = ERROR_CODE_COMM_ERROR;
		goto EndGetAdc;
	}
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		for (i = 0; i < iReadDataLen / 2; i++) {
			...
			FOCAL_TEST_PRINT("%d,   ", pRevBuffer[i]);
		}

		FOCAL_TEST_PRINT("\n");
	} else
		FOCAL_TEST_DBG("Comm_Base_IIC_IO error. error:%d.\n", ReCode);

EndGetAdc:

	if (pDataSend != NULL) {
		focal_free(pDataSend);
		pDataSend = NULL;
	}
