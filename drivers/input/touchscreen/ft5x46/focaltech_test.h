...
extern struct i2c_client *focal_i2c_client;
extern int focal_i2c_write(struct i2c_client *client,
			   char *writebuf, int writelen);
extern int focal_i2c_read(struct i2c_client *client,
...
#define FOCAL_TEST_DBG(fmt, args...) \
	pr_debug("[FOCAL] %s. line: %d.  "fmt"\n",\
			__func__, __LINE__, ##args)
#define FOCAL_TEST_PRINT(fmt, args...) printk(""fmt, ## args)
#define FOCAL_TEST_ERR(fmt, args...) \
	pr_err("[FOCAL] %s. line: %d.  "fmt"\n", \
			__func__, __LINE__, ##args)
#else
#define FOCAL_TEST_DBG(fmt, args...) do {} while (0)
#define FOCAL_TEST_PRINT(fmt, args...) do {} while (0)
#endif
