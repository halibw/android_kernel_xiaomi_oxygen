	...
	for (i = 0; i < pdata->cfg_size; i++, firmware++) {
		if (vid == firmware->vendor) {
			if (ft5x46->dev->of_node)
				if ((ft5x46->chip_id == firmware->chip) && (ft5x46->lockdown_info[1] == firmware->lcd_vendor)) {
					dev_info(ft5x46->dev, "chip id = 0x%x, lcd_vendor = 0x%x found it!\n",
						ft5x46->chip_id, ft5x46->lockdown_info[1]);
