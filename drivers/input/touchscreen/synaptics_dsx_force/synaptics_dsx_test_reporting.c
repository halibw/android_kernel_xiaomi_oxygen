...
static int test_do_preparation(void)
{
	...
	default:
		...
		if (!rmi4_data->hw_if->board_data->rt20_use_cbc) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < tx_num * rx_num; i++) {
		*(td43xx_rt95_part_one + i) =
			(*(td43xx_rt95_part_one + i) > TDDI_SHORT_LIMIT_B) ? 1 : 0;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	int rx_num = f54->rx_assigned;
	bool result = 1;
	const unsigned int noise_upper_limit = TDDI_NOISE_LIMIT;
	...
	if (f54->swap_sensor_side) {
		...
		for (i = 0; i < tx_num; i++) {
			for (j = 0; j < rx_num; j++) {
				...
				if ((temp > 0) && temp < TDDI_OPEN_TEST_LIMIT_PHASE2_LOWER) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	control.reg_99->integration_duration_lsb = TDDI_OPEN_TEST_INT_DUR_ONE;
	control.reg_99->integration_duration_msb = (TDDI_OPEN_TEST_INT_DUR_ONE >> 8) & 0xff;
	...
	control.reg_99->integration_duration_lsb = TDDI_OPEN_TEST_INT_DUR_TWO;
	control.reg_99->integration_duration_msb = (TDDI_OPEN_TEST_INT_DUR_TWO >> 8) & 0xff;
	control.reg_99->reset_duration = original_data_f54_ctrl99[2];
	retval = synaptics_rmi4_reg_write(rmi4_data,
			control.reg_99->address,
			control.reg_99->data,
...
static ssize_t syna_selftest_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	snprintf(buf, sizeof(f54->result_type), "%d", f54->result_type);
