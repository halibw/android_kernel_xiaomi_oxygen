...
static void synaptics_rmi4_switch_mode_work(struct work_struct *work)
{
	...
	if (value >= INPUT_EVENT_WAKUP_MODE_OFF && value <= INPUT_EVENT_WAKUP_MODE_ON) {
		...
		if (rmi4_data->suspend && !rmi4_data->chip_is_tddi)
		...
		unsigned int type, unsigned int code, int value)
{
	...
	if (type == EV_SYN && code == SYN_CONFIG) {
		...
		if (value >= INPUT_EVENT_START && value <= INPUT_EVENT_END) {
			ms = (struct synaptics_rmi4_mode_switch *)kmalloc(sizeof(struct synaptics_rmi4_mode_switch), GFP_ATOMIC);
			if (ms != NULL) {
				ms->data = rmi4_data;
		...
		bool enable)
{
	...
	if (enable)
		reporting_control = F12_WAKEUP_GESTURE_MODE;
	else
		reporting_control = F12_CONTINUOUS_MODE;
	retval = synaptics_rmi4_reg_write(rmi4_data,
			fhandler->full_addr.ctrl_base + offset,
			&reporting_control,
			sizeof(reporting_control));
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (bdata->cut_off_power || rmi4_data->chip_is_tddi) {
